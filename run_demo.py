...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the YCB Video Models (or set env YCB_MODEL_PATH)",
    )
    parser.add_argument(
    ...
    if ycb_model_path is not None:
        gt_mesh_file = os.path.join(
            ycb_model_path, "models", "006_mustard_bottle", "textured_simple.obj"
        )
    ...
    if gt_mesh_file is not None and os.path.isfile(gt_mesh_file):
        ...
        print(
            "[WARN] YCB model not found. Set --ycb_model_path or YCB_MODEL_PATH to compute Chamfer distance."
        )
    ...
    results.append(
        {"Object": obj, "Object_Number": obj_num, "Chamfer_Distance": chamfer_dis}
    )
