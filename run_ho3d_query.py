...
if __name__ == "__main__":
    ...
    overall_means = {
        "ADD-S": np.mean([np.mean(object_metrics[obj]["ADD-S"]) for obj in obj_folder])
        * 100,
        "ADD": np.mean([np.mean(object_metrics[obj]["ADD"]) for obj in obj_folder])
        * 100,
        "AR": np.mean([np.mean(object_metrics[obj]["AR"]) for obj in obj_folder]) * 100,
        "MSSD": np.mean([np.mean(object_metrics[obj]["MSSD"]) for obj in obj_folder])
        * 100,
        "MSPD": np.mean([np.mean(object_metrics[obj]["MSPD"]) for obj in obj_folder])
        * 100,
        "VSD": np.mean([np.mean(object_metrics[obj]["VSD"]) for obj in obj_folder])
        * 100,
    }
    ...
    df.to_excel(
        f"{save_results_est_path}/0_mean_all_metrics_classes_results.xlsx", index=False
    )
    ...
    df_all_frames = pd.concat(
        [df_all_frames, pd.DataFrame([means_all])], ignore_index=True
    )
