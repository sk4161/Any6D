...
class Ho3dReader:
    ...
    def get_mask(self, i):
        ...
        if "evaluation" in self.color_files[i]:
            ...
            mask = cv2.imread(
                f"{self.ho3d_root}/masks_XMem/{video_name}/{index:05d}.png", -1
            )
        else:
            mask = cv2.imread(
                f"{self.ho3d_root}/train/{video_name}/seg/{i:04d}.png", -1
            )[..., 1]
            mask = cv2.resize(
                mask,
    ...
    def get_occ_mask(self, i):
        ...
        index = int(os.path.basename(self.color_files[i]).split(".")[0])
        mask = cv2.imread(
            f"{self.ho3d_root}/masks_XMem/{video_name}_hand/{index:04d}.png", -1
        )
        return mask
    ...
    def get_gt_mesh(
        ...
        mesh = trimesh.load(
            f"{model_dir}/models/{self.get_video_name_full()}/textured_simple.obj"
        )
        return mesh
